...
public class AccountDBTest {
    ...
    Voorlopig hier geplaatst omdat ik dit had in mijn project, kan nog van pas komen?
    
    // Positieve test: account inloggen
    @Test
    public void testInloggenAccount(){
        try{
            accountDB.toevoegenAccount(account);
            Account inlogAcc = accountDB.inloggenAccount("miekedefoort", "wachtwoord123");
            
            assertEquals("Defoort", inlogAcc.getNaam());
            assertEquals("Mieke", inlogAcc.getVoornaam());
            assertEquals("miekedefoort", inlogAcc.getLogin());
            assertEquals("wachtwoord123", inlogAcc.getPaswoord());
            assertEquals("miekedefoort@hotmail.com", inlogAcc.getEmailadres());
            assertEquals(Geslacht.V, inlogAcc.getGeslacht());
        }catch(DBException ex){
            System.out.println("testInloggenAccount - " + ex);
        }
    }
    
    // Positieve test: inloggen met verkeerde login
    @Test
    public void testInloggenAccountVerkeerdLogin(){
        try{
            accountDB.toevoegenAccount(account);
            Account inlogAcc = accountDB.inloggenAccount("defoortmieke", "wachtwoord123");
    ...
    // Positieve test: inloggen met verkeerde paswoord
    @Test
    public void testInloggenAccountVerkeerdPaswoord(){
        try{
            accountDB.toevoegenAccount(account);
            Account inlogAcc = accountDB.inloggenAccount("miekedefoort", "verkeerdwachtwoord");
